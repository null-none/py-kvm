...
from .exceptions import KvmError, TimeoutException
...
_MAPPING = json.loads(
    "".join([line for line in open(__MAPFILE).readlines() if not line.startswith("#")])
)
...
def pprint(value):
    return {
        key: (
            ...
            else [pprint(elt) for elt in val] if isinstance(val, list) else val
        )
...
def gen_uuid():
    ...
    return "-".join(
        (
            "".join([random.choice(_CHOICES) for _ in range(0, 8)]),
...
def from_xml(elt, force_lists=[]):
    ...
    **elt** must be of type ``lxml.etree.Element``."""
    ...
    if not attrs and not childs and not text:
        ...
        elts = (
            OrderedDict(("@%s" % attr, value) for attr, value in attrs)
            if attrs
            else OrderedDict()
        )
        for child in childs:
...
def _stats(lines, ignore=False):
    return {
        elts[1 if ignore else 0]: elts[2 if ignore else 1]
        for line in lines
        if line
...
def __add_method(obj, method, conf):
    ...
    def stats_method(self, *args, **kwargs):
        with self._host.set_controls(parse=True, ignore_opts=ignore_opts):
            ...
            return _stats(
                self._host.virsh(cmd, *args, **kwargs), conf.get("ignore", False)
            )
    ...
    def tune_method(self, *args, **kwargs):
        ...
        if not kwargs or (
            len(kwargs) == 1 and any(opt in kwargs for opt in ignore_opts)
        ):
...
def Hypervisor(host, uri=None):
    ...
    for property_name, property_methods in _MAPPING.items():
        property_obj = type(
            "_%s" % str(property_name).capitalize(), (object,), dict(__init__=__init)
        )
    ...
    with self._host.set_controls(parse=True):
        ...
        for elt, elt_entries in sysinfo.items():
            ...
            result.update(
                {
                    elt: (
                        [entry(value["entry"]) for value in elt_entries]
